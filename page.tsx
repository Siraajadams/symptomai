...
export default function Home() {
  ...
  const whatsappText = encodeURIComponent(`SymptomAI referral request for ${form.name}. Outcome: ${result?.title}. Phone: ${form.phone}. Location: ${form.location}.`);
      ...
      {result ? (
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 20 }}>
            {result.level === 'EMERGENCY' && (
              <>
                ...
                <a className="button secondary" href={whatsappUrl}>Request paramedic / admin callback</a>
              </>
            )}
            {result.level === 'DOCTOR_IN_PHARMACY' && (
              <>
                <a className="button" href={carelinkUrl} target="_blank">Book doctor via Carelink</a>
                <a className="button secondary" href={whatsappUrl}>WhatsApp referral note</a>
              </>
            )}
            {(result.level === 'PHARMACIST_CARE' || result.level === 'SELF_CARE') && (
              <a className="button secondary" href={whatsappUrl}>Send advice / follow-up note</a>
            )}
            <button className="button" onClick={() => { setResult(null); setForm(initialForm); }}>New triage</button>
          </div>
        </div>
      ) : (
        <section className="hero">
          <div className="card">
            ...
            <p>Capture basic details, screen red flags, and route patients to emergency care, doctor in pharmacy, pharmacist care, or self-care.</p>
            <a className="button" href="#triage">Start triage</a>
          </div>
          <div className="card">
            <h2>Built for pharmacies</h2>
            <p>No diagnosis. No complex EHR. Just safe red-flag triage, referral support, and basic analytics.</p>
          </div>
        </section>
      )}
        ...
        <div id="triage" className="card" style={{ marginTop: 24 }}>
          ...
          <div className="grid">
            ...
            <label>Mobile / WhatsApp
              <input className="input" value={form.phone} onChange={e => update('phone', e.target.value)} placeholder="+27..." />
            </label>
            <label>Email
              <input className="input" value={form.email} onChange={e => update('email', e.target.value)} placeholder="Optional" />
            </label>
            <label>Nearest pharmacy / location
              <input className="input" value={form.location} onChange={e => update('location', e.target.value)} placeholder="Pharmacy or suburb" />
            </label>
            <label>Age
              <input className="input" type="number" value={form.age} onChange={e => update('age', e.target.value)} placeholder="Age in years" />
            </label>
            <label>Pregnant?
