...
export default function AdminPage() {
  ...
  const stats = useMemo(() => {
    const base: any = {
      ...
      EMERGENCY: 0,
      DOCTOR_IN_PHARMACY: 0,
      PHARMACIST_CARE: 0,
      SELF_CARE: 0,
    };

    records.forEach((r) => {
      base[r.decision.level] = (base[r.decision.level] || 0) + 1;
    });
  ...
  return (
    <main className="container">
      ...
      <div className="grid" style={{ marginTop: 24 }}>
        ...
        <div className="card">
          <h2>{stats.DOCTOR_IN_PHARMACY}</h2>
          <p>Doctor in pharmacy</p>
        </div>

        <div className="card">
          <h2>{stats.PHARMACIST_CARE + stats.SELF_CARE}</h2>
          <p>Pharmacy/self-care</p>
        </div>
      </div>
          ...
          <tbody>
            {records.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  <b>{r.decision.level.replaceAll('_', ' ')}</b>
                </td>
              </tr>
            ))}
          </tbody>
